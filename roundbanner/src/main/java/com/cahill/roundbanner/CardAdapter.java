...
public class CardAdapter extends RecyclerView.Adapter<BannerViewHolder> {
    ...
    public BannerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (dataCount != 0 && bannerAdapter != null) {
            ...
            return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.banner_item, parent, false));
        }
    }

    @Override
    public void onBindViewHolder(BannerViewHolder holder, int position) {
        holder.itemView.setTag(R.id.key_position, position % (data != null ? data.size() : dataCount));
        ...
        if (dataCount != 0 && bannerAdapter != null) {
            ...
            VH.mainTitle.setText(data.get(position % data.size()).getMainTitle());
            VH.mainTitle.setTextSize((float) mainTitleTextSize);
            VH.subtitleTitle.setText(data.get(position % data.size()).getSubtitleTitle());
            VH.subtitleTitle.setTextSize((float) subtitleTitleTextSize);
            Glide.with(context)
                    .load(data.get(position % data.size()).getImage()).centerCrop()
